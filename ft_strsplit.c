...
/*   Updated: 2014/01/09 13:56:21 by dtortera         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char			**ft_strsplit(char const *s, char c)
{
	...
	if ((token = s))
	{
		...
		if ((big_array = (char**)malloc((nb_tok + 1) * sizeof(char*))))
		{
			...
			while ((token_next = ft_strchr(token, c)))
			{
				if (token_next - token > 0)
					big_array[i++] = ft_strsub(s, token - s, token_next - token);
				token = token_next + 1;
			}
			if ((token_next = ft_strchr(token, '\0')) - token > 0)
				big_array[i++] = ft_strsub(s, token - s, token_next - token);
		}
	}
	return (big_array);
